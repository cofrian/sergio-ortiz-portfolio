...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="site-shell">
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            ...
            <Link className="button button-secondary" href={localePath(locale, "/ask")}><Search aria-hidden="true" size={16} />{locale === "es" ? "Preguntar al portfolio" : "Ask my portfolio"}</Link>
          </div>
        </div>
      ...
      <section className="section-block">
        <div className="section-heading"><div><p className="eyebrow">02 · Capabilities</p><h2>{locale === "es" ? "Explora por capacidad." : "Explore by capability."}</h2></div><Link className="button button-ghost" href={localePath(locale, "/connections")}>{locale === "es" ? "Ver conexiones" : "View connections"}<ArrowRight aria-hidden="true" size={16} /></Link></div>
      ...
      <section className="section-block">
        <div className="section-heading"><div><p className="eyebrow">03 · GitHub</p><h2>{locale === "es" ? "Actividad reciente." : "Recent activity."}</h2></div><span className="status-line"><span className="status-dot" />{locale === "es" ? "Bootstrap verificado" : "Verified bootstrap"}</span></div>
      ...
      <section className="section-block about-grid">
        ...
        <div><p className="eyebrow">05 · About</p><h2 className="display" style={{ fontSize: "clamp(3.2rem, 6vw, 6.5rem)" }}>{locale === "es" ? "Me interesa lo que ocurre después de que el modelo funciona." : "I care about what happens after the model works."}</h2><p className="muted" style={{ fontSize: "1.1rem", lineHeight: 1.75 }}>{profile.bio[locale]}</p><Link className="button button-primary" href={localePath(locale, "/about")}>{locale === "es" ? "Sobre mí" : "More about me"}<ArrowRight aria-hidden="true" size={16} /></Link></div>
      </section>
    </div>
  );
}
